...
@File: examples\occhs_time_series_predict\extract_data_for_industry_tp.py
@Software: vscode
@Description:
        提取数据进行各行业职业病确诊情况的时间序列预测
"""
...
if __name__ == "__main__":
    ...
    columns_names = [
        "id", "report_card_id", "organization_industry_type",
    ...
    diagnoise_groupby_df["exam_num"] = init_df.groupby(["organization_industry_type", "report_issue_date"])["diagnoise_res"].size()
    diagnoise_groupby_df["diagnoise_num"] = init_df.groupby(["organization_industry_type", "report_issue_date"])["diagnoise_res"].sum()
    diagnoise_groupby_df.reset_index(inplace=True)
    diagnoise_groupby_df.to_csv(output_path/"diagnoise_time_series_data_industry.csv", header=True, index=False, encoding="utf-8-sig")
