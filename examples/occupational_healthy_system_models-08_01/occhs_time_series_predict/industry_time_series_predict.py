...
INDUSTRY_TYPE_NAMES = {
    "卫生材料及医药用品制造": "medical_manufacture",
    # "计算机零部件制造": "computer_manufacture",
}
...
def plot_forecast_res(models_path,
    ...
    for col in hazard_types:
        data = pd.read_csv(output_path / 
                       f"{CITY_NAMES.get(city)}-industry_{INDUSTRY_TYPE_NAMES.get(industry_type)}-hazard_{HAZARD_TYPE_NAMES.get(col)}-data.csv",
        ...
        open(
            models_path /
            f"{CITY_NAMES.get(city)}-industry_{INDUSTRY_TYPE_NAMES.get(industry_type)}-hazard_{HAZARD_TYPE_NAMES.get(col)}-model.pkl",
            "rb"))
    ...
    ax.set_title(f"{city}-{industry_type}部分危害因素暴露情况及决策影响")
    ax.legend(loc="upper left")
    fig.tight_layout()
    fig.savefig(pictures_path /
                    f"{CITY_NAMES.get(city)}-industry_{industry_type}-hazard_exposure-res.png")
    plt.close(fig=fig)
    return fig
...
def train_step(input_path, models_path, output_path):
    ...
    for city in city_list:
        ...
        for industry_type in INDUSTRY_TYPE_NAMES.keys():
            hazard_sub_city_industry_df = hazard_sub_city_df.query("organization_industry_type == @industry_type")
            hazard_type_list = hazard_sub_city_industry_df["hazard_type"].drop_duplicates()
            ...
            # for hazard_type in hazard_type_list:
                ...
                hazard_sub_df = hazard_sub_city_industry_df.query("hazard_type == @hazard_type")
                # 统计城市-行业-危害因素类型下参加职业健康检查的人数
                ...
                if hazard_timeseries_sub_df.shape[0] > 10:
                    ...
                    for col in train_y.columns:
                        ...
                        pickle.dump(
                            ...
                            open(
                                models_path /
                                f"{CITY_NAMES.get(city)}-industry_{INDUSTRY_TYPE_NAMES.get(industry_type)}-hazard_{HAZARD_TYPE_NAMES.get(hazard_type)}-model.pkl",
                                "wb"))
                ...
                diagnoise_timeseries_output_df.to_csv(
                    output_path / 
                    f"{CITY_NAMES.get(city)}-industry_{INDUSTRY_TYPE_NAMES.get(industry_type)}-hazard_{HAZARD_TYPE_NAMES.get(hazard_type)}-data.csv",
...
if __name__ == "__main__":
    ...
    if task == "plot":
        city = "杭州市"
        industry_type = "卫生材料及医药用品制造"
        hazard_types = ["噪声","高温(高温作业)"]
