# -*- coding: utf-8 -*-
"""
@DATE: 2024-01-31 16:54:26
@Author: Liu Hengjiang
@File: examples\\NOISH_1998_studying-01_26\Chinese_classifier_prob.py
@Software: vscode
@Description:
...
if __name__ == "__main__":
    ...
    logger.add(f"./log/Chinese_classifier_prob-{datetime.now().strftime('%Y-%m-%d')}.log",level="INFO")
    ...
    parser.add_argument("--plot_types", type=list, default=["ROC", "TPR", "PDF"])
    ...
    if task.startswith("analysis"):
        ...
        input_df = extract_df.query(
            "age_box in ('A-1','A-2','A-3') and duration_box in ('D-1', 'D-2', 'D-3')")[[
                key_feature, "age_box", "duration_box", "HL1234_Y"
            ]]
        ...
        if task.split("-")[-1] == "train":
            ...
            pickle.dump(model, open(models_path / f"Chinese_catboost_classifier_model-{key_feature}.pkl", "wb"))
        else:
            model = pickle.load(open(models_path / f"Chinese_catboost_classifier_model-{key_feature}.pkl", "rb"))
        ...
        if "TPR" in plot_types:
            ...
            plt.savefig(pictures_path / f"FPR-{key_feature}_fig.png")
            # plt.show()
            plt.close()
        if "PDF" in plot_types:
            prob_df = pd.DataFrame()
            fig, ax = plt.subplots(1, figsize=(6.5, 5))
            for age_box, duration_box in product(("A-1","A-2","A-3"),("D-1","D-2","D-3")):
            ...
            plt.savefig(pictures_path / f"group_prob-{key_feature}_fig.png")
            plt.close(fig=fig)
            prob_df.to_csv(output_path / f"{key_feature}-prob_df.csv", header=True, index=False)
            ...
            ax.set_xlabel("Sound Level in dB")
            plt.legend(loc="best")
            plt.savefig(pictures_path / f"average_prob-{key_feature}_fig.png")
            plt.close(fig=fig)
