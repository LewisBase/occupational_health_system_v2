...
@File: examples\\NOISH_1998_studying-01_26\\NOISH_experiment_group_classifier_prob_catboost.py
@Software: vscode
@Description:
        针对NOISH, 1998数据的catboost分类模型概率密度曲线
"""
...
if __name__ == "__main__":
    ...
    logger.add(f"./log/NOISH_experiment_group_classifier_prob_catboost-{datetime.now().strftime('%Y-%m-%d')}.log",level="INFO")
    ...
    if task.startswith("analysis"):
        ...
        if task.split("-")[-1] == "train":
            ...
            pickle.dump(model, open(models_path / f"NOISH_experiment_group_catboost-{key_feature}_classifier_model.pkl", "wb"))
        else:
            model = pickle.load(open(models_path / f"NOISH_experiment_group_catboost-{key_feature}_classifier_model.pkl", "rb"))
        ...
        if "ROC" in plot_types:
            ...
            plt.legend(loc="best")
            plt.title("ROC Curve")
            plt.savefig(pictures_path / f"NOISH_experiment_group_catboost-{key_feature}_ROC_fig.png")
            # plt.show()
            plt.close()
        if "TPR" in plot_types:
            ...
            plt.savefig(pictures_path / f"NOISH_experiment_group_catboost-{key_feature}_TPR_fig.png")
            # plt.show()
            plt.close()
        if "PDF" in plot_types:
            ...
            plt.savefig(pictures_path / f"NOISH_experiment_group_catboost_prob-{key_feature}_fig.png")
            plt.close(fig=fig)
            prob_df.to_csv(output_path / f"NOISH_experiment_group_catboost_prob-{key_feature}_df.csv", header=True, index=False)
            ...
            ax.set_xlabel("Sound Level in dB")
            plt.legend(loc="best")
            plt.savefig(pictures_path / f"NOISH_experiment_group_catboost_average_prob-{key_feature}_fig.png")
            plt.close(fig=fig)
