...
@File: examples\\NOISH_1998_studying-01_26\Chinese_control_group_logistic_regression_1.py
@Software: vscode
@Description:
...
def logistic_func(params, x):
    ...
    F = alpha + beta1 * x[:, 0] + beta21 * x[:, 1] + beta22 * x[:, 2] + beta23 * x[:, 3]
...
def userdefine_logistic_regression_task(
        ...
        params_init: list = [-4.18049946, 0.07176393, 1.32653869, 2.13749184, 8.65684751],
        **kwargs,
):
    """_summary_
    自定义逻辑回归函数
...
def userdefine_logistic_regression_plot(
    ...
    duration_matrix = np.tile(duration, (len(age), 1))
    plot_X = np.concatenate((age[:, np.newaxis], duration_matrix),axis=1)
    pred_y = logistic_func(x=plot_X, params=best_params_estimated)
    fig, ax = plt.subplots(1, figsize=(6.5, 5))
    ax.plot(age, pred_y, alpha=0.4)
    x_min, x_max = ax.get_xlim()
    y_min, y_max = ax.get_ylim()
    key_point_y = logistic_func(x=np.array([np.concatenate([[key_point_x], duration])]), params=best_params_estimated)[0]
    ...
    ax.set_title(f"Duration {duration_desp}")
...
if __name__ == "__main__":
    ...
    logger.add(
        f"./log/Chinese_control_group_logistic_regression_1-{datetime.now().strftime('%Y-%m-%d')}.log",
        level="INFO")
    ...
    if task == "analysis":
        ...
        # 使用全部数据
        ## NIHL1234_Y
        # fit_df = extract_df[["age", "duration_box_best", "NIHL1234_Y"]]
        # userdefine_logistic_regression_task(
        #     fit_df=fit_df,
        #     models_path=models_path,
        #     model_name="Chinese_control_group_udlr_model_1.pkl",
        #     y_col_name="NIHL1234_Y",
        #     params_init=[-84.51745217,0.27847085,36.00289889,67.61945703,66.59146369],
        #     minimize_bounds = ([None,None],[None,None],[30,45],[45,60],[60,75])
        # )
