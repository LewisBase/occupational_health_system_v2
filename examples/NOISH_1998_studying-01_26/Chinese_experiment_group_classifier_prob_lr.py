# -*- coding: utf-8 -*-
"""
@DATE: 2024-03-12 10:46:13
@Author: Liu Hengjiang
@File: examples\\NOISH_1998_studying-01_26\Chinese_experiment_group_classifier_prob_lr.py
@Software: vscode
@Description:
        针对中国工人实验组数据的lr分类模型概率密度曲线
"""
...
if __name__ == "__main__":
    ...
    logger.add(f"./log/Chinese_experiment_group_classifier_prob_lr-{datetime.now().strftime('%Y-%m-%d')}.log",level="INFO")
    ...
    if task == "extract":
        ...
        filter_df.to_csv(output_path / "Chinese_extract_experiment_classifier_df.csv",
                         header=True,
                         index=True)
    if task.startswith("analysis"):
        ...
        if task.split("-")[-1] == "train":
            ...
            pickle.dump(model, open(models_path / f"Chinese_experiment_group_lr-{key_feature}_classifier_model.pkl", "wb"))
        else:
            model = pickle.load(open(models_path / f"Chinese_experiment_group_lr-{key_feature}_classifier_model.pkl", "rb"))
        ...
        if "ROC" in plot_types:
            ...
            plt.legend(loc="best")
            plt.title("ROC Curve")
            plt.savefig(pictures_path / f"Chinese_experiment_group_lr-{key_feature}_ROC_fig.png")
            # plt.show()
            plt.close()
        if "PDF" in plot_types:
            ...
            plt.savefig(pictures_path / f"Chinese_experiment_group_lr_prob-{key_feature}_fig.png")
            plt.close(fig=fig)
            prob_df.to_csv(output_path / f"Chinese_experiment_group_lr_prob-{key_feature}_df.csv", header=True, index=False)
            ...
            ax.set_xlabel("Sound Level in dB")
            plt.legend(loc="best")
            plt.savefig(pictures_path / f"Chinese_experiment_group_lr_average_prob-{key_feature}_fig.png")
            plt.close(fig=fig)
