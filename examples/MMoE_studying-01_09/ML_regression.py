# -*- coding: utf-8 -*-
"""
@DATE: 2024-01-10 10:04:14
@Author: Liu Hengjiang
@File: examples\MMoE_studying-01_09\ML_regression.py
@Software: vscode
@Description:
        进行基于catboost的特征筛选结果验证与特征交互
        
        1. 在带有C计权计算结果的数据集上发现使用特征组合
        'max_Peak_SPL_dB',
        ...
        时取得的效果最佳，测试集上的MAE=8.02；
        2. 旧的计算数据中没有C计权的信息，去掉C计权相关特征后的特征组合
        'LAeq_adjust_segment_ari_A+A',
        'LAeq_adjust_segment_ari_A+A max_Peak_SPL_dB', 
        'max_Peak_SPL_dB'
        此时测试集上的MAE=8.10；
        3. 采用完全一致的设置在2778条数据上进行实验(替换为A+n)，得到的测试集MAE=11.27；
...
def get_importance_from_regressor(X: pd.DataFrame,
    ...
    logger.info(f"Model {y.name}: MAE = {round(MAE,4)}")
...
if __name__ == "__main__":
    ...
    logger.add(f"./log/ML_regression-{datetime.now().strftime('%Y-%m-%d')}.log",level="INFO")
    ...
    if task == "analysis":
        ...
        model, MAE, importances_res = get_importance_from_regressor(
            ...
            poly_degree=1)
        pickle.dump(model, open(model_path/"branchmark.pkl", "wb"))
